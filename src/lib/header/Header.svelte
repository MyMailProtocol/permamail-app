<script lang="ts">
  ...
  import { getPrivateKey, getLatestVersionTxid } from '$lib/myMail';
  import Arweave from 'arweave';
  import { goto } from '$app/navigation';
  import config from '$lib/arweaveConfig';

  let arweave: any = Arweave.init(config);
  ...
  onMount(async () => {
    ...
    const latestVersion: string = await getLatestVersionTxid(arweave);
    console.log(`got latest version ${latestVersion}`);

    const activeVersion: string = window.location.pathname.split('/')[1];
  ...
  async function doTransaction() {
    ...
    const transaction: any = await arweave.createTransaction({ data: 'asdf' }, wallet);
    await arweave.transactions.sign(transaction, wallet);
    console.log(transaction);

    const uploader: any = await arweave.transactions.getUploader(transaction);
  ...
  async function testAuth() {
    ...
    const result = await fetch(`http://localhost:5000/Mail/authToken?address=${address}`, {
    ...
    const pk: CryptoKey = await getPrivateKey(wallet);
    const byteData: Uint8Array = b64UrlToBuffer(data);
